...
test("Cap values match expected literals", () => {
  // Safety net for the re-export-from-source testdata pattern (Resolved decision #8): the caps are
  // imported from the sim, but an import alone can't flag a silent change to a behavior-defining
  // number. An intentional cap change updates both the sim constant and these literals; an
  // accidental one trips here. (No page needed — a pure check of the imported constants.)
