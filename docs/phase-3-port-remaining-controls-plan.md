...
- **Touch-target sizing for the small inline toggles (`<Switch>`, `<Checkbox>`)** (Phase 6 hardening). Their clickable label renders ~20px tall — fine on width (the label), but short of the 44px `$touch-target-min` convention and at/under the WCAG 2.5.8 (AA) 24×24 minimum on the height axis. The substantial controls (`<Button>`, `<NumberField>`, `<Select>`) already meet 44px via their real height. A hit-area-expansion overlay (transparent `::after`) was prototyped in Phase 3 but reverted: in the dense Starter controls layout a full 44px overlay **overlaps** the neighboring control row (~4px into the ≥8px gaps), which risks mis-taps — and the safe expansion amount depends on the *consumer's* layout gaps, which the shared component can't know. Deferred so it's solved holistically in the Phase 6 a11y pass (alongside axe-core / Lighthouse), where the real sims' layouts are known and a consistent approach (modest non-overlapping expansion to clear the 24px AA floor, slightly larger visible controls, or per-layout spacing) can be chosen against actual designs. No designer spec for the checkbox/switch exists yet, which is another reason to wait.
