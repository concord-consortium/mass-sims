...
- **Vite + S3 publicPath (validated, with dynamic-publicPath support added):** Vite's `base: "./"` covers the per-version case (HTML and JS co-located in `version/<tag>/<sim>/`). For the top-level *promoted* release pattern (`/mass-sims/<sim>/index.html` referencing JS still living in `/mass-sims/version/<tag>/<sim>/`), we use Vite's `experimental.renderBuiltUrl` plus a runtime asset-base helper in each sim's `main.tsx` — the Vite equivalent of Webpack's `publicPath: "auto"`. The full pattern is described in the "Top-level release promotion via index-top.html" subsection below.
...
Each sim's `vite.config.ts` configures `experimental.renderBuiltUrl` to emit a runtime expression for JS-emitted URLs:
...
Each sim's `main.tsx` sets up the runtime helper using `import.meta.url` — which at runtime refers to the URL of the actual bundle, regardless of where the HTML lives:
...
Now whether the HTML is at `/mass-sims/version/v1.2.3/sim-one/` (per-version) or `/mass-sims/sim-one/` (promoted), the JS computes asset URLs relative to *itself* and they always resolve correctly.

**Caveat about import ordering.** ES module imports are hoisted, so the setup code above runs *after* the imports execute. For placeholder sims with no asset imports, that's harmless. As soon as a real sim does `import iconUrl from "./icon.png"` (or transitively imports anything that does), the setup must move to a *separate* file imported as the *first* import in `main.tsx`:

```ts
// src/_asset-url-setup.ts  — name chosen to sort first alphabetically
const assetBase = new URL("..", import.meta.url).href;
Object.assign(globalThis, { __assetUrl: (f: string) => assetBase + f });
```

```ts
// src/main.tsx
import "./_asset-url-setup";  // MUST be first — runs before any asset import
import { StrictMode } from "react";
// ...
```

When that refactor is needed will be obvious — a 404 on the first asset.
...
- **`dist/index-top.html`** — paths rewritten to `../version/<tag>/<sim>/assets/main-abc.js`. Deployed (renamed to `index.html`) to the top-level `<sim>/index.html` for promotion.
...
#### The CI-side promotion logic

`ci.yml` adds a "Promote to top-level URL" step that runs only on tag pushes:

- Copies each sim's `dist/index-top.html` to `s3://.../mass-sims/<sim>/index.html`.
- Copies the root `index.html` (which already uses relative `./<sim>/` links) to `s3://.../mass-sims/index.html`.

Branch pushes skip this step — they only do the per-version deploy. This means every tag push is also a release (gets promoted). If we ever want pre-release tags (e.g., `v*-pre*`) to skip promotion, the trigger condition narrows.
