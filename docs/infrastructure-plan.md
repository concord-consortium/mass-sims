...
> **Status (Phase 1 build):** `Section` and the `SimulationFrame` compound component (header + Trials/Simulation/Data slots + three-column flex grid + accessible info modal) are built to the §3 API and exported from the shared barrel; the four-width × 562 render check is satisfied by the dev-only width preview (`/__preview`, served by every sim's dev server — see Phase 5). Utility hooks, tokens, and `global.scss` are in place. **Deferred:** final Section/chip/region visual treatment and designer-tuned slot proportions. Styling is plain (global) SCSS scoped under a per-component root class (the verified house convention — see the corrected Styling note in §1); the info-modal prop is `infoModalContent`.
...
- **Phase 2a — Shared library rebuild against the demo design (✅ COMPLETE).** Token rewrite (`tokens.scss`, `global.scss`), partner-branding SVGs, restructured `<SimulationFrame>` (50 px title bar, draggable About panel, no project bar), new `<TrialCard>` and `<DataSubsection>` components, the frame-preview workspace updated (since superseded by the width preview — see Phase 5).
...
**Phase 5 — Width preview (✅ COMPLETE, MAS-22)**
Every sim's dev server serves a dev-only `/__preview` route that renders *that* sim in `<iframe>` cards at the four target widths × 562 — a real, interactive sim, not placeholder content. It ships as `widthPreviewPlugin()` inside `createSimViteConfig()`, so a new sim gets it with no setup, and `apply: "serve"` keeps it out of every production build. The page also flags what an iframe otherwise hides: content that overflows its allocation, hard-clipped text, and elements escaping the frame. This replaced the old frame-only `packages/sim-frame-preview` workspace, which `packages/starter` (a real sim exercising the whole shared surface) previews strictly better.

> **Dropped:** this phase was originally scoped as a *multi-sim* harness workspace (`packages/sim-test-harness`) that would load **every** sim at the four widths at once, pointed at deployed branch URLs in CI. Dropped as low-value once the per-sim preview shipped: it covers the case developers actually hit (checking the sim they're working on), and a cross-sim survey adds a workspace to maintain for a view nobody needs day to day. If visual-regression coverage is wanted later, Playwright screenshots against the existing four-width project matrix are the cheaper route than a bespoke harness.
...
9. **`docs/deployment.md`** (Phase 6 deliverable) covers building and deploying to an arbitrary static host, written with a DESE-led handover in mind.
