...
- **Vite + S3 publicPath (validated in Phase 0):** Vite's `base: "./"` is the working pattern for our versioned-folder deploys. Each built bundle resolves its assets relative to its own `index.html`, so a single build artifact can be served from any path on S3 without rebuilding. The dynamic-publicPath complexity from FOSS's Webpack config (HTML at one level, JS in a versioned subfolder via `__webpack_public_path__`) is *not* needed for our case — the relative-path pattern is sufficient. If a future deploy scheme separates HTML and JS into different version subfolders, revisit using `experimental.renderBuiltUrl` (Vite 6) or runtime `__VITE_BASE__` injection.
