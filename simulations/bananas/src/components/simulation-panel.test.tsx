...
// The Fungus switch (and Select/Button) auto-emit via useLogEvent → lara-interactive-api's
// log(action, data). Mock that transport so the fungus_set toggle sequence can be asserted.
const { log } = vi.hoisted(() => ({ log: vi.fn() }));
vi.mock("@concord-consortium/lara-interactive-api", () => ({ log }));

import { SimulationPanel } from "./simulation-panel";

// Drives a parent <Select> the way a user would: open the trigger, click an option.
// The trigger's accessible name includes its label ("Parent 1" / "Parent 2"), so the
// two dropdowns and the three control buttons are all addressable by name.
...
describe("SimulationPanel", () => {
  it("renders the selectors, offspring grid, and controls with NO status pill", () => {
    const { getByLabelText, getByRole, queryByRole } = render(<SimulationPanel />);
  ...
  it("shows the crosses/offspring stats pill after a cross", () => {
    const { getByRole } = render(<SimulationPanel rng={seededRandom("state2")} />);
    selectBothParents(getByRole);
  ...
  it("appends Fungus active to the stats pill when fungus is on for a cross", () => {
    const { getByRole } = render(<SimulationPanel rng={seededRandom("state2f")} />);
    selectBothParents(getByRole);
    fireEvent.click(getByRole("switch", { name: "Fungus" }));
    fireEvent.click(getByRole("button", { name: "Cross Plants" }));
    expect(getByRole("status")).toHaveTextContent(
  ...
  it("replaces the parent selectors with chips after the first cross", () => {
    const { getByRole, queryByRole, getByText } = render(
      <SimulationPanel rng={seededRandom("lock")} />,
    );
    selectBothParents(getByRole);
  ...
  it("toggles the Fungus switch only with both parents and no crosses, logging each change", () => {
    const { getByRole, queryByRole } = render(<SimulationPanel />);
  ...
  it("locks the Fungus switch after the first cross and freezes fungusOn", () => {
    const { getByRole } = render(<SimulationPanel rng={seededRandom("t6-lock")} />);
    selectBothParents(getByRole);
  ...
  it("keeps the Fungus lock through subsequent crosses up to the cap", () => {
    const { getByRole, container } = render(<SimulationPanel rng={seededRandom("t6-cap")} />);
    selectBothParents(getByRole);
    fireEvent.click(getByRole("switch", { name: "Fungus" })); // on
    for (let i = 0; i < 6; i++) {
      fireEvent.click(getByRole("button", { name: "Cross Plants" }));
  ...
  it("Reset Trial clears fungusOn and releases the lock", () => {
    const { getByRole } = render(<SimulationPanel rng={seededRandom("t6-reset")} />);
    selectBothParents(getByRole);
    fireEvent.click(getByRole("switch", { name: "Fungus" })); // on
    fireEvent.click(getByRole("button", { name: "Cross Plants" }));
  ...
  it("Reset after a cross clears the offspring, unlocks the selectors, and releases the Fungus lock", () => {
    const { getByRole, queryByRole } = render(<SimulationPanel rng={seededRandom("t7-cross")} />);
    selectBothParents(getByRole);
    fireEvent.click(getByRole("button", { name: "Cross Plants" }));
  ...
  it("Reset is disabled and emits nothing from a fully empty State 0", () => {
    const { getByRole } = render(<SimulationPanel />);
  ...
  it("shows only the placeholder hint (no fungus marker) with zero crosses and fungus off", () => {
    const { getByRole, queryByText } = render(<SimulationPanel />);
    expect(getByRole("list", { name: /crosses/i })).toHaveTextContent(
  ...
  it("renders the 'Fungus introduced' marker above the placeholder when fungus is on before crossing", () => {
    const { getByRole, getByText } = render(<SimulationPanel />);
    selectBothParents(getByRole);
  ...
  it("renders one cross row labelled A1 with 5–20 plants after a cross", () => {
    const { getByRole, getAllByRole, container } = render(
      <SimulationPanel rng={seededRandom("t8-one")} />,
    );
    selectBothParents(getByRole);
    fireEvent.click(getByRole("button", { name: "Cross Plants" }));
  ...
  it("accumulates rows A1, A2, A3 in order across three crosses", () => {
    const { getByRole, getAllByRole } = render(<SimulationPanel rng={seededRandom("t8-three")} />);
    selectBothParents(getByRole);
  ...
  it("renders the Fungus marker above the first cross row (DOM order)", () => {
    const { getByRole, getByText } = render(<SimulationPanel rng={seededRandom("t8-marker")} />);
    selectBothParents(getByRole);
    fireEvent.click(getByRole("switch", { name: "Fungus" })); // on
    fireEvent.click(getByRole("button", { name: "Cross Plants" }));
  ...
  it("shows the max-crosses placeholder and disables Cross Plants + Fungus at the cap", () => {
    const { getByRole, getAllByRole } = render(<SimulationPanel rng={seededRandom("t8-cap")} />);
    selectBothParents(getByRole);
  ...
  it("renders every plant infected when an all-rr cross is made under fungus", () => {
    ...
    const { getByRole, container } = render(<SimulationPanel rng={() => 0.7} />);
  ...
  it("never infects resistant offspring even under fungus", () => {
    ...
    const { getByRole, container } = render(<SimulationPanel rng={() => 0.2} />);
    selectBothParents(getByRole);
    fireEvent.click(getByRole("switch", { name: "Fungus" })); // on
    fireEvent.click(getByRole("button", { name: "Cross Plants" }));
  ...
  it("never renders a between-row 'introduced after' marker", () => {
    const { getByRole, queryByText } = render(
      <SimulationPanel rng={seededRandom("t8-regression")} />,
    );
    selectBothParents(getByRole);
    fireEvent.click(getByRole("switch", { name: "Fungus" })); // on
    for (let i = 0; i < 6; i++) fireEvent.click(getByRole("button", { name: "Cross Plants" }));
  ...
  it("gives each cross row an aria-label describing its counts", () => {
    const { getByRole, getAllByRole } = render(<SimulationPanel rng={seededRandom("t8-aria")} />);
    selectBothParents(getByRole);
  ...
  it("scrolls the offspring grid to the bottom after a cross", () => {
    const { getByRole, container } = render(<SimulationPanel rng={seededRandom("scroll")} />);
    selectBothParents(getByRole);
